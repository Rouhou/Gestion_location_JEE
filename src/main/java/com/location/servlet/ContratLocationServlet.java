...
@WebServlet(name = "ContratLocationServlet",
        urlPatterns = {"/proprietaire/contrats", "/proprietaire/contrat/*"})
public class ContratLocationServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest req, HttpServletResponse resp)
        ...
        // Liste de tous les contrats du propriétaire : GET /proprietaire/contrats
        if ("/proprietaire/contrats".equals(servletPath)) {
            Utilisateur proprietaire = (Utilisateur) req.getSession().getAttribute("utilisateur");
            List<ContratLocation> contrats = contratService.findByProprietaire(proprietaire.getId());
            req.setAttribute("contrats", contrats);
            req.getRequestDispatcher("/WEB-INF/views/proprietaire/contrats.jsp").forward(req, resp);

        // Détail d'un contrat : GET /proprietaire/contrat/{id}
        } else if (pathInfo != null && pathInfo.matches("/\\d+")) {
            Long id = Long.parseLong(pathInfo.substring(1));
            contratService.findById(id).ifPresent(c -> req.setAttribute("contrat", c));
            req.getRequestDispatcher("/WEB-INF/views/proprietaire/contrat-detail.jsp").forward(req, resp);

        // Formulaire nouveau contrat : GET /proprietaire/contrat/new
        } else {
            // Unités disponibles + locataires pour les selects
            req.setAttribute("unitesDisponibles",
                    uniteService.findDisponibles());
            req.setAttribute("locataires",
                    utilisateurService.findByRole(Utilisateur.RoleUtilisateur.LOCATAIRE));
            req.getRequestDispatcher("/WEB-INF/views/proprietaire/contrat-form.jsp").forward(req, resp);
        }
    }
    ...
    protected void doPost(HttpServletRequest req, HttpServletResponse resp)
        ...
        resp.sendRedirect(req.getContextPath() + "/proprietaire/contrats");
    }
}

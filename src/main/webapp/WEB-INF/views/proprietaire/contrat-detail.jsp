...
<head>
    ...
    <title>Détail contrat — GestionLoc</title>
        ...
        <div class="topbar">
            ...
            <div style="display:flex;gap:.6rem">
                ...
                <c:if test="${contrat.statut eq 'ACTIF'}">
                    <form method="post" action="${pageContext.request.contextPath}/proprietaire/contrats">
        ...
        <div class="page-body">
            ...
            <div style="display:grid;grid-template-columns:1fr 1fr;gap:1.3rem">
                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        <table style="width:100%;font-size:.9rem">
                            <tr>
                                <td style="color:#9e9e9e;padding:.5rem 0;width:45%">Statut</td>
                                <td>
                                    <c:choose>
                                        <c:when test="${contrat.statut eq 'ACTIF'}">
                                            <span class="badge badge-green">✅ Actif</span>
                                        </c:when>
                                        <c:when test="${contrat.statut eq 'TERMINE'}">
                                            <span class="badge badge-gray">Terminé</span>
                                        </c:when>
                                        <c:otherwise>
                                            <span class="badge badge-red">🚫 Résilié</span>
                                        </c:otherwise>
                                    </c:choose>
                                </td>
                            </tr>
                            <tr>
                                <td style="color:#9e9e9e;padding:.5rem 0">Date de début</td>
                                <td><strong>${contrat.dateDebutFormattee}</strong></td>
                            </tr>
                            <tr>
                                <td style="color:#9e9e9e;padding:.5rem 0">Date de fin</td>
                                <td>
                                    <c:choose>
                                        <c:when test="${not empty contrat.dateFin}">
                                            ${contrat.dateFinFormattee}
                                        </c:when>
                                        <c:otherwise><span style="color:#bdbdbd">Non définie</span></c:otherwise>
                                    </c:choose>
                                </td>
                            </tr>
                            <tr>
                                <td style="color:#9e9e9e;padding:.5rem 0">Loyer convenu</td>
                                <td>
                                    <strong style="color:#1a237e;font-size:1.05rem">
                                        <fmt:formatNumber value="${contrat.loyerConvenu}" pattern="#,##0"/> F CFA
                                    </strong>
                                </td>
                            </tr>
                            <tr>
                                <td style="color:#9e9e9e;padding:.5rem 0">Dépôt de garantie</td>
                                <td>
                                    <c:choose>
                                        <c:when test="${not empty contrat.depotGarantie}">
                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        <%-- Locataire --%>
                        <p style="font-size:.75rem;color:#9e9e9e;text-transform:uppercase;letter-spacing:.05em;margin-bottom:.4rem">
                            Locataire
                        </p>
                        ...
                        <p style="color:#9e9e9e;font-size:.85rem">
                            📍 ${contrat.unite.immeuble.adresse}, ${contrat.unite.immeuble.ville}
                        </p>
                        <p style="color:#9e9e9e;font-size:.85rem">
                            🛏️ ${contrat.unite.nbPieces} pièce(s) —
                            📐 <fmt:formatNumber value="${contrat.unite.superficie}" pattern="#,##0.##"/> m²
                        </p>
                    </div>
                </div>
            </div>
            ...
            <div class="card" style="margin-top:1.3rem">
                <div class="card-header">
                    ...
                    <c:if test="${contrat.statut eq 'ACTIF'}">
                        <a href="${pageContext.request.contextPath}/locataire/paiement/new?contratId=${contrat.id}"
                ...
                <div class="table-wrap">
                    <table>
                        ...
                        <c:forEach var="p" items="${contrat.paiements}">
                            <tr>
                                <td><strong>${p.moisConcerne}</strong></td>
                                <td>
                                    <strong><fmt:formatNumber value="${p.montant}" pattern="#,##0"/> F CFA</strong>
                                </td>
